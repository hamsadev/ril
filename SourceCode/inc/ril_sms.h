...
#define RIL_SMS_PHONE_NUMBER_MAX_LEN                                                               \
    (LIB_SMS_PHONE_NUMBER_MAX_LEN +                                                                \
     2) // It may contain '+' at the first position and will contain '\0' at the end.
...
typedef struct {
    uint8_t alpha; // It's value is same as 'LIB_SMS_DCSAlphaEnum'
    ...
    uint8_t data[RIL_SMS_TEXT_DATA_MAX_LEN];
    uint32_t length;
...
typedef struct {
    uint8_t status; // It's value is same as 'Enum_RIL_SMS_StatusType'
    uint8_t type;   // It's value is same as 'LIB_SMS_PDUTypeEnum'
